...
Comboタブは、Host Link Config RPCの`COMBO` featureから設定済みComboを読み取り、物理キーマップ上で構成キーを表示します。編集モードでは`追加`または設定済みComboを選び、2〜8個のキー、`bindings`、対象レイヤー、`timeout-ms`、`slow-release`、`require-prior-idle-ms`を編集します。設定済みComboは一覧の各枠にある削除ボタンから削除できます。重複するキー集合が重なるレイヤーで使われている場合は、実機へ送る前に拒否します。`適用`はRAM上のCombo tableへの反映であり、再起動後も残すには画面下部の`保存`が必要です。

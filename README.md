...
- Codex CLI の起動。SettingsからWindowsまたはWSLのプロジェクトを選び、保存操作なしでCodex CLIを専用のWindows Terminalウィンドウで開けます。ランチャー設定だけが起動時に自動保存されます。
...
- 設定: 外観、Polling、HID、起動時の挙動、Codex連携とCodex CLIランチャーを設定します。

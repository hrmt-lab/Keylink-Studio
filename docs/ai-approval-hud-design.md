...
- 状態: **段階1・段階2ともに実装・実機確認済み（2026-09-06、実HIDで受け入れ完了）。** 次は段階4の一部前倒し（ScreenKeyの9バイト拡張とフォーカス枠・リング演出、§8）。その後に段階3（Claude Codeのdecision経路）
- 作成日: 2026-09-03
- 最終更新: 2026-09-06（段階2の実HID受け入れ完了にともない §6・§8・§11〜§14 を実装に合わせて改訂）
...
[決める] ✅ を押す ／ エンコーダを回して選び ✅ を押す
   ...
   ▼
[復帰] 押した ScreenKey が 0.6 秒だけ確定表示を出し、HUD が消える
       作業画面は最初から一度も動いていない
```

複数が同時に待機しているときは、答えたいセッションの **ScreenKey を押す**と HUD の内容が
それに切り替わる。どれが選ばれているかは ScreenKey のフォーカス枠で分かる（§8）。
...
|---|---|---|
| 0 | 通常 | 従来どおり |
| 1 | 待機中・**HUD の対象** | フォーカス枠（下記） |
| 2 | 待機中・対象ではない | 従来の黄点滅のまま |
| 3 | 回答を送信した直後 | リング＋チェックの 0.6 秒アニメーション |
...
```text
 ▛▔▔        ▔▔▜     四隅に L 字マーク（黄 0xFACC15、太さ 6px、腕 28px 程度）
 ▏             ▕     背景に黄を不透明度 15% 程度で敷く
 ░   Codex     ░     ロゴは従来どおり中央
 ▏             ▕     ★点滅しない
 ▙▁▁        ▁▁▟
...
カメラのオートフォーカス枠の語彙である。本質は色や太さではなく **点滅を止めること**で、
「**動いている＝未処理、静止している＝いま相手にしている**」という対比を作る。
色や太さの違いより、周辺視野では動きの有無のほうが強く効く。

### 送信直後（値 3）＝ リングとチェック

合計 0.6 秒。緑 `0x22C55E` のリングが 0.25 秒で一周描かれて閉じ、その直後にチェックの
ストロークが左下→右上へ 0.15 秒で引かれ、残り約 0.2 秒保持してから通常へ戻る。

**タイマーは Firmware が持つ。** Host は値 3 を 1 回送るだけで、0.6 秒後に送り直さない。
...
| 項目 | 変更 |
|---|---|
| `AI_CLIENT_STATE` | 8 → **9バイト**（末尾に ScreenKey 表示状態 1バイト）。**未実装** |
| capability | **bit 14** `AI_CLIENT_SCREENKEY_STATE` を新設。**未実装** |
...
| **4** | ScreenKey の 9バイト拡張とフォーカス枠・リング演出（**種別アイコンと `APP_LAYER` は取りやめ**） | 気づきやすさ |
...
ScreenKey の 9バイト拡張とフォーカス枠・リング演出は、**段階3 より先に着手する**。
段階2 で ScreenKey が「対象を選ぶキーなのにフィードバックを返さない」状態が実機で明らかに
なったため（§8）。種別アイコンと `APP_LAYER` 切替は段階4 から取り除いた。
...
8. **ScreenKey に「対象」表示が無い状態で複数キーボードを同時接続したときの見え方。**
   同報方式（§8）では、同じ表示スロットを映す画面はすべて対象表示になる。
   実運用で ScreenKey 付きが 2 台以上同時接続する場面が出てから再評価する

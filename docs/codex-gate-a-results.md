...
- 履歴上の最終試行ハーネスSHA-256: `B925CB507B4E1A77AF5F8E293A4E01CF494F6035433061E5E26A26E361213EDC`
- 整理後の再検証ハーネスSHA-256: `0ADC58197E1DB72EECA87F5308C8D06D602DBEEC77218010F7BE8451AA38E387`
...
Broker採用理由を再検証できる最小構成として、`Resume`と`Approval`だけを残した。
整理後のハーネスでは、同じThread ID、Validation Pair ID、ハーネスSHA-256を使用することを強制する。

- `Resume`の再現成功status: `passed`
- `Approval`の否定条件再現成功status: `broker_required`
- `Approval`の最終decision: `observer_ineligible_response_required_request_delivered`
- Observerのexit intent／OS exit code 42は安全停止診断として記録するが、Broker必須判定条件にはしない

`ThreadTurn`、`LateObserver`、`UserInput`、`McpElicitation`、pending request切断シナリオは、
最終判定に不要であるためコードから削除した。未確認であること自体は本書の記録として維持する。

整理後の`PrepareOnly`では、Codex CLI 0.144.6、正式なCLI option、生成Schema、
Schema SHA-256、`target/gate-a/`のGit除外を確認済みである。モデル呼び出しを伴うResume／Approvalの再実行は行っていない。

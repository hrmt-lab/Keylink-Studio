...
- Combo一覧には設定済みslotだけを縦に表示し、名前、動作、対象レイヤーと各Comboの削除ボタンを置く。新規作成は一覧下部の`追加`から空きslotへ進む。
...
- Tauri／UI実装完了: Combo DTO／commandとHost Link workerを追加し、Keymap Viewer編集モードへ`Keymap`／`Combo`切替、slot一覧、物理キー位置の選択、既存Behavior Pickerによる`bindings`選択、Apply、個別DELETEを追加した。ComboのRAM dirtyは共通編集バーへ統合し、保存・破棄・`.keymapに戻す`はCombo専用ボタンを設けず共通操作から実行する。フォームの未適用draft中は共通操作を無効化し、slot移動時に破棄確認を行う。
...
- UI read-only確認完了: Cornixの設定済み6件をComboタブへ表示し、一覧、キー構成、動作、レイヤー、各propertyを実画面で確認した。重複Comboの適用はHost側事前validationで拒否し、firmwareの`INVALID_ARGUMENT`を技術コードのまま見せないUIへ変更した。Key Pressなどの動作は`&kp`等の短いZMK表記、キー位置はbinding labelと数値positionを併記する。設定済みComboだけの縦一覧、`追加`、枠内削除、`Group`アイコン、property tooltip、Behavior Picker検索を反映した。

次の未実装範囲:

1. UI経由のmutation／永続化確認。baselineを保存し、slot 31だけでSET→dirty→DISCARD→SET→SAVE→再接続後復元→DELETE→SAVE→再接続後baseline復元を行う。
2. Comboを既存`-keymap.json` Export / Restoreへ追加し、feature別preview／resultを実装する。
3. 最終回帰、利用者向け文言整理、release準備。

次はUI経由のmutation／永続化確認から始める。既存slot 0〜5を変更せず、空きslot 31だけを使う。Firmware基準点は`zmk-raw-hid`の`43d9e4b feat: support 64-byte Host Link packets`と、`zmk-rawhid-app`の`fa60c14 fix: defer Combo saves from Raw HID callbacks`。Firmware pushは未実施である。

...
| --- | --- | --- | --- |
| `main` | `v2` | Host Link v2 Common Header / 64 byte packet 対応 firmware | `HOST_HELLO` / `DEVICE_HELLO` / 既存 packet を v2 layout で送受信。Config RPC type は予約のみ |
...
| Config RPC | packet type 予約のみ | 未実装 | `CONFIG_REQUEST` / `CONFIG_RESPONSE` は typed decode 未対応 |
...
- Config RPC と encoder 編集 API は次フェーズの対象です。現時点で packet type は予約済みですが request/response 管理は実装していません。

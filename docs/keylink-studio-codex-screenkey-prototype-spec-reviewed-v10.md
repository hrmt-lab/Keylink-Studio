...
ScreenKey単体プロトタイプ完了後の次の機能追加は、Turn中の
「考えている」と「何かを実行している」を区別する表示細分化とする。
2026-07-26時点では方針決定と文書化のみで、実装には着手していない。

| 論理的な状況 | ScreenKey表示 | 方針 |
|---|---|---|
| 入力待ち（`WAITING_INPUT`） | オレンジの外周をゆっくり呼吸 | 現在の黄色点滅から変更し、承認待ちと区別する |
| 推論中 | 現行`WORKING`と同じ青色の外周をゆっくり呼吸 | 「考えている」を示す |
| コマンド／ツール実行中 | 現行`WORKING`の青い外周移動線 | 現在の表示を維持する |
| Web／ファイル検索中 | 現行`WORKING`の青い外周移動線 | 現在の表示を維持する |

`CONNECTING`、`RECONNECTING`、`INTERRUPTED`／`CANCELLED`、`RETRYING`は
個別の表示状態を追加せず、現行のReducerと表示を維持する。
承認待ち、完了、エラー、セッションなし、利用可能の表示も変更しない。

この細分化では、既存の`WORKING`との後方互換性を維持する。
推論中、コマンド／ツール実行中、Web／ファイル検索中を
Host内部とHost Linkでどう表現するかは実装開始時に設計し、
App Serverの構造化イベントだけから判定する。テキスト本文から推測しない。
オレンジの色値、呼吸周期、短時間の状態変化に対するdebounceは
実装時に定義し、ScreenKey実機で確定する。

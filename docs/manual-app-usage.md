...
1. `アクションID`（0〜255）を入力します。キーボード側 `&host_action <ID> <値>` の `<ID>` と一致させます。
2. `動作` を選びます。選べる動作は次のとおりです。
   - `このアプリの画面を開く`: RawHID Host のウィンドウを前面に表示します。
   - `監視を停止`: 監視を停止します。
   - `AI 使用量を更新`: AI 使用量を再取得します。
   - `アプリを起動`: 指定した実行ファイルを起動します。
3. `アプリを起動` を選んだ場合は、`実行ファイル` に起動する exe のパスを入力します（例: `C:\tools\example.exe`）。
4. `追加` を押します。すでに同じアクションIDがある場合や、起動でパス未指定の場合はエラーになります。

設定済みのバインディングは一覧に表示され、ゴミ箱ボタンで削除できます。アクションはキーボードに割り当てた `<値>` を path やコマンドとして解釈しません。実行内容はこの画面で設定したものだけです。
...
- `Show window`: ウィンドウを再表示します。
- `Quit`: アプリを完全に終了します。

アプリはシングルインスタンスです。すでに起動している状態でもう一度起動した場合は、新しいウィンドウは開かず、既存のウィンドウが前面に表示されます。

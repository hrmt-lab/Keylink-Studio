...
編集モードでは、既存Comboを選ぶか`追加`を押して編集します。キーマップ上で2〜8個のキーを選び、`bindings`、`layers`、`timeout-ms`、`slow-release`、`require-prior-idle-ms`を設定して`適用`します。動作の選択には通常キーと同じPickerを使い、Key Pressは`&kp`のようなZMK表記で表示します。重複するキー集合を重なるレイヤーへ登録することはできません。

`適用`は実機RAMへの反映です。再起動後も残すには画面下部の`保存`を押してください。`変更を破棄`は保存済みCombo tableへ戻します。削除は一覧内の各Comboにある削除ボタンから行い、削除後も永続化には`保存`が必要です。フォームを変更してまだ`適用`していない間は、共通の保存・破棄操作が無効になります。

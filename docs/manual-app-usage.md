...
編集モードでは、既存Comboを選ぶか`追加`を押して編集します。キーマップ上で2〜8個のキーを選び、`bindings`、`layers`、`timeout-ms`、`slow-release`、`require-prior-idle-ms`を設定して、フォーム右下の`適用`を押します。動作の選択には通常キーと同じPickerを使い、Key Pressは`&kp`のようなZMK表記で表示します。重複するキー集合を重なるレイヤーへ登録することはできません。
...
- 未保存のまま他の画面へ移動しようとした場合は、移動前に `保存して移動` / `破棄して移動` / `キャンセル` を選びます。キー書き込み待ちや保存 / 破棄 / 終了処理中は、処理が終わるまで保存して移動 / 破棄して移動は選べません。

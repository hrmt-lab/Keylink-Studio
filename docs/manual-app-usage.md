...
編集モードの `.keymap に戻す` は、ZMK Studio で保存したキー、レイヤー、物理レイアウトを削除し、firmware に書き込まれた `.keymap` の状態へ戻します。同じキーボードの Host Link が接続されている場合は、Keylink Studio で保存したエンコーダ override も削除し、`.keymap` の `sensor-bindings` を使う状態へ戻します。

この操作は戻せず、未保存の編集も失われます。Bluetooth pairing、Host Link identity、Keylink Studio のアプリ設定、キー統計は削除しません。Host Link が接続されていない場合は通常キーとレイヤーだけを戻し、エンコーダ設定は変更しません。Studio側とエンコーダ側は別通信のため、片方だけ失敗した場合は画面のエラーを確認して同じ操作を再試行してください。

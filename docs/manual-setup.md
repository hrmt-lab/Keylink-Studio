...
- ZMK Studio Keymap Viewer は ZMK Studio USB serial RPC transport を必要とします。BLE transport は v1 の対象外です。編集には実機側で `&studio_unlock` を実行して Studio を unlocked にする必要があります。Host 側から unlock は行いません。

...
### Combo編集を使うファームウェアの設定

Keylink StudioからComboを編集する場合は、`.keymap`の`combos`ノードに`status = "disabled";`を指定してください。Combo定義は初期値として`zmk-rawhid-app`が読み取りますが、ZMK標準Combo listenerは無効化し、編集・保存に対応したmodule側のruntime listenerだけを使用します。
...
`CONFIG_RAWHID_APP_COMBO_RUNTIME=y`の状態で`/combos`が有効な場合、二つのCombo listenerが同時に組み込まれるのを防ぐため、ビルドは次のメッセージで停止します。

```text
static assertion failed: "CONFIG_RAWHID_APP_COMBO_RUNTIME=y requires /combos status = \"disabled\""
```

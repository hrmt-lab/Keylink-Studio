...
UI では、Dashboard と Devices 画面の Host Link 一覧を `device_uid_hash` 単位のキーボード表示として扱います。同じ `device_uid_hash` の USB / BLE HOG endpoint が同時に見えている場合は 1 台に集約し、接続中の transport アイコンを並べて表示します。Devices 画面では集約カード内に各 endpoint の HID path を表示します。`device_uid_hash` が取得できない endpoint は誤結合を避けるため path 単位で個別に扱います。
...
- `BATTERY_STATUS` の `level = 0xFF` は unknown として扱い、UI では `--%`、tray では `?` と表示します。`0` は有効な 0% として扱います。
...
- Keymap Viewer は独立した ZMK Studio RPC client モジュールを使います。通常表示では USB serial / CDC ACM または BLE Studio transport から snapshot を取得します。編集モードでは `StudioEditSession` が USB serial または BLE Studio transport を保持して `set_key_at` / layer management / save / discard を実行します。BLE Studio 編集では UI 側でキー書き込みを 1 件ずつ queue 処理し、pending 中は保存 / 破棄 / 編集終了 / レイヤー構造操作を止めます。未保存変更がある状態で他画面へ移動しようとした場合は、UI が遷移を止めて `保存して移動` / `破棄して移動` / `キャンセル` を提示します。pending 中は保存して移動 / 破棄して移動も止めます。書き込み失敗時は未送信 queue を破棄し、復旧用の `studio_abort_edit` で編集セッションを破棄してから再読み込みします。
...
- Keymap backup は `schema = "rawhid-host.keymap-backup"` / `schema_version = 1` の JSON です。推奨拡張子は `-keymap.json` です。復元の真実は各 binding の `position`, `behavior_id`, `param1`, `param2` で、表示用の label と検証用の behavior 名も保持します。秘密情報、設定ファイル内容、ユーザー絶対パスは含めません。

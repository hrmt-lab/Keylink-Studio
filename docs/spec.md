...
- Keymap Viewer は独立した ZMK Studio RPC client モジュールを使います。通常表示では USB serial / CDC ACM または BLE Studio transport から snapshot を取得します。編集モードでは `StudioEditSession` が USB serial または BLE Studio transport を保持して `set_key_at` / layer management / save / discard を実行します。BLE Studio 編集では UI 側でキー書き込みを 1 件ずつ queue 処理し、pending 中は保存 / 破棄 / 編集終了 / レイヤー構造操作を止めます。書き込み失敗時は未送信 queue を破棄し、復旧用の `studio_abort_edit` で編集セッションを破棄してから再読み込みします。
- Keymap backup は `schema = "rawhid-host.keymap-backup"` / `schema_version = 1` の JSON です。推奨拡張子は `.rawhid-keymap.json` です。復元の真実は各 binding の `position`, `behavior_id`, `param1`, `param2` で、表示用の label と検証用の behavior 名も保持します。秘密情報、設定ファイル内容、ユーザー絶対パスは含めません。

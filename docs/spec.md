...
- Keymap Viewer は独立した ZMK Studio RPC client モジュールを使います。通常表示では read-only snapshot を取得し、編集モードでは `StudioEditSession` が USB serial 接続を保持して `set_key_at` / layer management / save / discard を実行します。

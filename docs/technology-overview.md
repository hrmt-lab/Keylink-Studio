...
RawHID Host は、Windows で常駐し、USB または BLE HOG で HID device として見える対応キーボードと通信するアプリです。
...
The ZMK firmware side is not included in this repository. Host Link features require the compatible HID receiver described in [Packet Specification](packet-spec.md), over either USB HID or BLE HOG when exposed through Windows HID APIs. ZMK Studio keymap features use ZMK Studio RPC over USB serial / CDC ACM.

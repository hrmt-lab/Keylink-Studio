...
RawHID Host は、Windows で常駐し、USB 接続された対応キーボードと通信するアプリです。
...
このリポジトリに含まれるのは PC 側のアプリです。Host Link の命令を受け取って layer や表示に反映する ZMK 側 firmware は、別途同じ packet protocol に対応している必要があります。ZMK Studio キーマップ機能は Host Link とは別経路で、ZMK Studio 対応 firmware を使います。
...
flowchart LR
    ...
    Rust --> HID["Raw HID / USB"]
    HID --> Keyboard["ZMK keyboard"]
    Rust <--> Studio["ZMK Studio RPC / USB serial"]
    Studio <--> Keyboard
```

画面で行った操作は Tauri を通じて Rust の処理に渡ります。Rust は設定を読み、Windows の状態を調べ、必要な packet をキーボードへ送ります。ZMK Studio キーマップ機能では、Raw HID ではなく USB serial / CDC ACM の Studio RPC client を使います。
...
| TOML | 設定ファイル形式 |
| Raw HID | PC からキーボードへ独自 packet を送る USB 経路 |
...
| `src/hid.rs` | USB device 探索、HELLO 検証、packet 送受信 |
...
Raw HID は PC からキーボードへ独自の 32 byte payload を送る USB 経路です。このアプリでは `HL` protocol と呼ぶ packet を使います。
...
編集中は USB serial port を保持するため、同じ port を別 command が二重に開かないように backend 側で `port_busy` を返します。同じ device の読み取りは保持中の session 経由で snapshot を返します。
...
RawHID Host is a resident Windows app. React and TypeScript build the UI, Tauri connects that UI to Rust, and Rust handles Windows integration, TOML configuration, Raw HID, packet encoding, monitoring, time sync, AI usage snapshots, uplink packets, and ZMK Studio keymap editing.

The ZMK firmware side is not included in this repository. Host Link features require the compatible Raw HID receiver described in [Packet Specification](packet-spec.md). ZMK Studio keymap features use ZMK Studio RPC over USB serial / CDC ACM.
